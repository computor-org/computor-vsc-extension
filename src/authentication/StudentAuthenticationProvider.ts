...
export class StudentAuthenticationProvider implements vscode.AuthenticationProvider, vscode.Disposable {
  private static readonly PROVIDER_ID = 'computor-student';
  private static readonly PROVIDER_LABEL = 'Computor Student';
  ...
  constructor(
    ...
    this.credentialStorage = new VscodeCredentialStorage(context, 'student');
    
    // Register the authentication provider
    this.disposables.push(
      vscode.authentication.registerAuthenticationProvider(
        StudentAuthenticationProvider.PROVIDER_ID,
        this.options.label || StudentAuthenticationProvider.PROVIDER_LABEL,
        this,
  ...
  async createSession(scopes: readonly string[]): Promise<vscode.AuthenticationSession> {
    const credentials = await this.showStudentLoginPrompt(scopes);
    if (!credentials) {
      throw new Error('User cancelled student authentication');
    }
  ...
  private async showStudentLoginPrompt(scopes: readonly string[]): Promise<ComputorCredentials | undefined> {
    void scopes;
    
    const settingsManager = await this.getSettingsManager();
    const settings = await settingsManager.getSettings();
    
    // Show student-specific login dialog
    const baseUrl = await vscode.window.showInputBox({
      title: 'Computor Student Authentication',
      prompt: 'Enter the backend API URL for students',
  ...
  private async createSessionFromCredentials(
    ...
    const account = {
      ...
      label: `Student: ${credentials.username} @ ${credentials.realm || 'default'}`
    };
    ...
    if (credentials.token) {
      ...
      const authData = {
        ...
        realm: credentials.realm || 'student',
  ...
  private async refreshSession(session: ComputorAuthenticationSession): Promise<ComputorAuthenticationSession> {
    if (session.credentials.token) {
      throw new Error('Student token expired. Please re-authenticate.');
    }
    ...
    throw new Error('Unable to refresh student session');
  }
      ...
      for (const sessionId of sessionIds) {
        ...
        if (credentials) {
          const session: ComputorAuthenticationSession = {
            ...
            account: {
              id: `student-${credentials.username || credentials.realm || 'default'}`,
              label: `Student: ${credentials.username || 'Token'} @ ${credentials.realm || 'default'}`
            },
            scopes: [],
            credentials: {
              ...credentials,
  ...
  static getProviderId(): string {
    return StudentAuthenticationProvider.PROVIDER_ID;
  }
}