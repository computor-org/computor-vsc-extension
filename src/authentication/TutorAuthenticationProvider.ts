...
export class TutorAuthenticationProvider implements vscode.AuthenticationProvider, vscode.Disposable {
  private static readonly PROVIDER_ID = 'computor-tutor';
  private static readonly PROVIDER_LABEL = 'Computor Tutor';
  ...
  constructor(
    ...
    this.credentialStorage = new VscodeCredentialStorage(context, 'tutor');
    
    // Register the authentication provider
    this.disposables.push(
      vscode.authentication.registerAuthenticationProvider(
        TutorAuthenticationProvider.PROVIDER_ID,
        this.options.label || TutorAuthenticationProvider.PROVIDER_LABEL,
        this,
  ...
  async createSession(scopes: readonly string[]): Promise<vscode.AuthenticationSession> {
    const credentials = await this.showTutorLoginPrompt(scopes);
    if (!credentials) {
      throw new Error('User cancelled tutor authentication');
    }
  ...
  private async showTutorLoginPrompt(scopes: readonly string[]): Promise<ComputorCredentials | undefined> {
    void scopes;
    
    const settingsManager = await this.getSettingsManager();
    const settings = await settingsManager.getSettings();
    
    // Show tutor-specific login dialog
    const baseUrl = await vscode.window.showInputBox({
      title: 'Computor Tutor Authentication',
      prompt: 'Enter the backend API URL for tutors',
    ...
    const username = await vscode.window.showInputBox({
      title: 'Tutor Login',
      prompt: 'Enter your tutor username',
    ...
    const password = await vscode.window.showInputBox({
      title: 'Tutor Login',
      prompt: 'Enter your tutor password',
      password: true,
  ...
  private async createSessionFromCredentials(
    ...
    const account = {
      id: `tutor-${credentials.username}`,
      label: `Tutor: ${credentials.username} @ ${credentials.realm || 'default'}`
    };
  ...
  private async refreshSession(session: ComputorAuthenticationSession): Promise<ComputorAuthenticationSession> {
    if (session.credentials.token) {
      throw new Error('Tutor token expired. Please re-authenticate.');
    }
    ...
    throw new Error('Unable to refresh tutor session');
  }
      ...
      for (const sessionId of sessionIds) {
        ...
        if (credentials) {
          const session: ComputorAuthenticationSession = {
            ...
            account: {
              id: `tutor-${credentials.username || credentials.realm || 'default'}`,
              label: `Tutor: ${credentials.username || 'Token'} @ ${credentials.realm || 'default'}`
            },
            scopes: [],
            credentials: {
              ...credentials,
  ...
  static getProviderId(): string {
    return TutorAuthenticationProvider.PROVIDER_ID;
  }
}