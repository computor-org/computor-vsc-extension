...
/**
 * Factory for creating virtual scrolling services
 */
export class VirtualScrollingFactory {
  private static instances: Map<string, VirtualScrollingService<any>> = new Map();
  
  /**
   * Get or create a virtual scrolling service
   */
  static getService<T>(
    key: string,
    loadFunction?: (page: number, pageSize: number) => Promise<{ items: T[]; total: number }>,
    config?: Partial<VirtualScrollConfig>
  ): VirtualScrollingService<T> {
    if (!this.instances.has(key) && loadFunction) {
      this.instances.set(key, new VirtualScrollingService<T>(loadFunction, config));
    }
    
    return this.instances.get(key) as VirtualScrollingService<T>;
  }
  
  /**
   * Clear a specific service
   */
  static clearService(key: string): void {
